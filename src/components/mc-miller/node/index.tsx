...
export const Node: FC<IProps> = ({
  ...
  const context = useContext(McMillerContext);
  ...
  return (
    <div onClick={onClick} className={classes}>
      <Checkbox
        color="primary"
        checked={
          status === McMillerStatus.Checked ||
          status === McMillerStatus.Intermediate
        }
        ...
        onChange={(_, checked) => {
          if (checked) {
            ...
            context.setChecked(
              context.checked.filter(
                x => !include(node.path, x) && !include(x, node.path),
              ),
            );
          }
        }}
